...
var DxD = new Class({
  ...
  setIni: function($elDrags, $elDrops){
	...
	$coco = this.container.pos;
        
    if ($elDrops)
      this.setIniDrops($elDrops);
  ...
  setIniDrops: function($elDrops){
    ...
    if (this.options.drop.randomPos)
      this.options.adjDrops.randomize();
    this.adjustDrops();
  },
  ...
  setIniDrags: function($elDrags){
    ...
    if ($type(this.options.adjDrags) == 'object')
      this.adjObject2Array();
    if (this.options.drag.randomPos) {
			var $randomOk;
			do {
				this.orderIndDrag = this.options.adjDrags.randomize();
				for (var $i = 0, $ind = this.orderIndDrag[0], $randomOk = ($i != $ind) ; $i < this.orderIndDrag.length; $ind = this.orderIndDrag[$i], $randomOk = ($randomOk || ($i != $ind)), $i++) {}
			} while (!$randomOk);
		};
  ...
  createDrag: function($objDrag, $options){
    ...
    if ($objDrag.node.getStyle('left') == 'auto' || $objDrag.node.getStyle('top') == 'auto')
      $objDrag.node.position($objDrag.node.getPosition($objDrag.node.offsetParent));
  ...
  drag: function($objDrag, $event){
    ...
    for (var z in this.options.drag.modifiers) {
      ...
      if (this.options.drag.limit && $objDrag.limit[z]) {
        ...
        if ($chk($objDrag.limit[z][0]) && ($objDrag.value.now[z] < $objDrag.limit[z][0])) {
          $objDrag.value.now[z] = $objDrag.limit[z][0];
        }
      };
      if (this.options.drag.grid[z])
        $objDrag.value.now[z] -= ($objDrag.value.now[z] % $objDrag.options.drag.grid[z]);
      if (this.options.drag.style)
        $objDrag.node.setStyle(this.options.drag.modifiers[z], $objDrag.value.now[z] + this.options.drag.unit);
      else
        $objDrag.node[this.options.drag.modifiers[z]] = $objDrag.node.value.now[z];
    };
  ...
  stop: function($objDrag, $event){
    ...
    if ($event)
      this.fireEvent('complete', $objDrag); // <-- ev.onComplete
  },
  ...
  checkAgainst: function($objD){
    var umbral = this.mouse.now;
    /*
    $('header').set('html',
                    '<p>this.mouse.now: <b>(' + this.mouse.now.x + ', ' + this.mouse.now.y + ')</b></p>' + 
                    '<p>this.mouse.start: <b>(' + this.mouse.start.x + ', ' + this.mouse.start.y + ')</b></p>' +
                    '<p>this.mouse.pos: <b>(' + this.mouse.pos.x + ', ' + this.mouse.pos.y + ')</b></p>' + 
                    '<p>start - pos: <b>(' + (this.mouse.start.x - this.mouse.pos.x) + ', ' + (this.mouse.start.y - this.mouse.pos.y) + ')</b></p><hr />' +
                    '<p>this.value.now: <b>(' + this.value.now.x + ', ' + this.value.now.y + ')</b></p>' +
                    '<p>this.value.umbral: <b>(' + this.value.umbral.x + ', ' + this.value.umbral.y + ')</b></p> <hr />' +
                    '<p>$objD.value.left: <b>' + $objD.value.left + '</b></p>'
                    );
*/
    
    return (umbral.x > $objD.value.left && umbral.x < $objD.value.right && umbral.y > $objD.value.top && umbral.y < $objD.value.bottom);
  ...
  levelDrags: function(){
    this.elDrags.each(function($objDrag, $iOB){
      if ($objDrag)
        $objDrag.node.setStyle('z-index', this.options.zIndexBottom + this.ordDrag[$iOB])
    }, this);
  },
  ...
  adjObject2Array: function(){
    // En este caso suponemos que los elementos tienen las mismas dimensiones. Sería interesante hacer el calculo del posicionamiento para anchos variables entre cada elementos.
    //var $distXEl = (this.options.adjDrags.width - this.elDrags[0].node.getComputedSize().totalWidth) / (this.options.adjDrags.elXLine);
    $distXEl = this.options.adjDrags.distXEl;
...
DxD.Base = new Class({
  ...
  setIniBase: function(){
    ...
    this.elDrops.each(function($objDrop, $iOB){
      ...
      if (this.options.multiDrop === false) {
      ...
      if ($type(this.options.multiDrop) == 'string' && this.options.multiDrop == 'wordLength') {
  ...
  trueDrop: function($objDrag, $objDrop){
    ...
    if ($objDrop.cntDrops == $objDrop.maxNumDrops) {
      ...
      if (!this.elDrops.length)
        this.fireEvent('trueDropsComplete');
    }
  },
  ...
  deleteDrag: function(drag){
    if (this.options.dragDelete) {
      if (this.options.dragDelete == 'none')
        drag.node.setStyle('display', 'none');
      else
        drag.node.destroy();
    };
  },
...
DxD.Swap = new Class({
  ...
  checkAgainst: function($overDrag){
  	var now = {
		x: this.mouse.now.x - $coco.x,
		y: this.mouse.now.y - $coco.y
		}
		
	var oNow = $overDrag.backPos;
		oNow.x2 = oNow.x + $overDrag.size.x;
		oNow.y2 = oNow.y + $overDrag.size.y;

//  	$('header').set ('html', '<p>drag: <b>' + now.x + '</b> | <b>' + now.y + '</b></p>');
	
//	console.debug ($umbralOverD);
	
	/*
    if ($overDrag === this || $overDrag.enabled === false)
      return false;
    ...
    */
	...
	return (now.x > oNow.x && now.x < oNow.x2 && now.y > oNow.y && now.y < oNow.y2);
  ...
  dropComplete: function($objDragOut, $objDragIn){
    ...
    if (this.initWord == this.word4Drags)
      this.fireEvent('wordOk', this.initWord);

    if (this.checkAllOrder()) this.fireEvent ('dragsAreOrdened');

  },

  checkAllOrder: function () {

    for ($i = 0, $ind = this.elDrags[0].ind; $i < this.elDrags.length; ++$i)
      if (this.elDrags[$i].ind - this.elDrags[$i].indDrop) return false;
    return true;
  },
